...
class PipelineRunner(PipelineEngineBridgeMixin, PipelineAgentTasksMixin):
    ...
    async def _execute_image(self, job_id: str) -> dict[str, Any]:
        ...
        if ep is None:
            raise ValueError("episode.json not found; run rw first (or manually seed it)")
        return await _ImageRun(runner=self, job_id=job_id, job_dir=job_dir, episode=ep).run()
...
class _RwRun:
    ...
    async def run_one(self, cand: dict[str, str]) -> None:
        try:
            res: Any = await _invoke_rw_candidate(
        ...
        if draft.get("status") == "success":
            ...
            try:
                draft.update(await asyncio.to_thread(
                    _apply_rw_qc, self.rw_root / cand["id"], cand["id"], self.on_progress,
                ))
    ...
    async def run(self) -> dict[str, Any]:
        self.on_progress(f"{len(MODEL_CANDIDATES)} 模型并行启动；source={len(self.source_text)} 字")
        ...
        await asyncio.gather(*[self.run_one(c) for c in MODEL_CANDIDATES])
        ...
        self.on_progress(f"完成：{success_count}/{len(MODEL_CANDIDATES)} 成功")
        return {
            "drafts": drafts_out,
