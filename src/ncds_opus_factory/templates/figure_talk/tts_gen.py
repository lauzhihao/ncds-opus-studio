...
照搬 stickman/tts_gen.py:figure_talk 的 beats.js 同样带 zh 字段(双引号),逻辑一致。
...
ZH_PATTERN = re.compile(r'\bzh"?\s*:\s*"((?:[^"\\]|\\.)*)"')  # 认裸键 zh: 与 JSON 键 "zh":


def load_zh() -> list[str]:
    """优先读结构化 beats.json(吴道子必产);无则正则兜底 beats.js。"""
    if BEATS_JSON.exists():
        ...
        return [b.get("zh", "") for b in beats if b.get("zh")]
    return [m.group(1) for m in ZH_PATTERN.finditer(BEATS_JS.read_text(encoding="utf-8"))]
...
def main() -> int:
    ...
    beats = load_zh()
    if not beats:
        print("No zh entries found in beats.json/beats.js", file=sys.stderr)
        return 2
