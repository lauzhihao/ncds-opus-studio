/* PoC 录屏器:headless Chrome 录 svg-rig 小人成片 + ffmpeg 合配音。
 * 复用 paper_card_talk/render.mjs 的逻辑,去掉 009 专属的 capZh/beats 耦合,
 * 改成只等 window.__player.startRecordingPlayback + 监听 body.ending。
 * 跑:  (cd state/stickman_poc && ln -sf /tmp/node_modules node_modules && node render_stick.mjs)
 */
...
const OUTPUT_MP4 = path.join(OUTPUT_DIR, 'stickman_poc.mp4');
const TMP_VIDEO = '/tmp/stick-silent.mp4';
const TMP_AUDIO = '/tmp/stick-audio.mp3';
const HTTP_PORT = 8770;
