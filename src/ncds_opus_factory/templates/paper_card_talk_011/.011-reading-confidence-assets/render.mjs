/* 离线渲染：把 011 跑成 1920×1080 30fps MP4，音频从 audio/*.mp3 合成接进去。
 *
 * 流程：
 ...
 *
 * 用法（仓库根目录）：
 *   node .011-reading-confidence-assets/render.mjs
 *
 * 输出：.011-reading-confidence-assets/output/011-reading-confidence.mp4
 *
 * 需要：
...
const OUTPUT_MP4 = path.join(OUTPUT_DIR, '011-reading-confidence.mp4');
const TMP_VIDEO = '/tmp/011-render-silent.mp4';
const TMP_AUDIO = '/tmp/011-render-audio.mp3';
const HTTP_PORT = 8765;
const URL_011 = `http://127.0.0.1:${HTTP_PORT}/011-reading-confidence.html`;
const FPS = 30;
...
async function buildAudioTrack() {
  ...
  const silenceIntro = '/tmp/011-silence-intro.mp3';
  const silenceGap = '/tmp/011-silence-gap.mp3';
  const silenceTail = '/tmp/011-silence-tail.mp3';
  ...
  const concatList = '/tmp/011-concat.txt';
...
async function main() {
  ...
  try {
    ...
    log('loading page');
    await page.goto(URL_011, { waitUntil: 'networkidle0', timeout: 30000 });
