...
(function () {
  ...
  const STYLE_CSS = `
    ...
    /* 退出编辑模式时存在 3-5 帧的"白色面板闪烁"竞态:
       1) exit() 移除 body.edit-mode 并把 __tweaks_mount/inspector-mount reparent 回 body
       2) .em-drawer 移除后 ".em-drawer .twk-panel { position:relative }" 失配
       3) .twk-panel 立刻回到默认 position:fixed; right:16px; bottom:16px (右下角)
       4) Inspector useEffect 后置发 __deactivate_edit_mode → setOpen(false) 才卸载
       studio 场景下面板只服务编辑模式; 这条规则只在 edit-mode.js 加载时存在,
       线上 ncds.cc 没探测到 edit-server 时不加载本脚本, 行为不变. */
    body:not(.edit-mode) .twk-panel { display: none !important; }
