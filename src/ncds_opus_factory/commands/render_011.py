"""/render_011 —— paper_card_talk_011 模板的一键渲染。
...
调用约定
--------
- episode_path     : 用户编辑后的 episode.json（pipeline runner 来自 03_rw 节点）
- audio_dir        : NNNN.mp3 目录（来自 05_tts 节点）
- picture_dir      : 图片目录，可空（沿用模板自带；来自 04_wst 节点产物）
- output_path      : 最终 MP4 落地路径
- template_dir     : 默认 src/.../templates/paper_card_talk_011/
...
DEFAULT_TEMPLATE_DIR = (
    _REPO_ROOT / "src" / "ncds_opus_factory" / "templates" / "paper_card_talk_011"
)
ASSETS_SUBDIR = ".011-reading-confidence-assets"
HTML_FILENAME = "011-reading-confidence.html"
...
def _build_workdir(
    ...
    workdir 结构（与模板一致）：
        workdir/
          011-reading-confidence.html
          .011-reading-confidence-assets/
...
def run(
    ...
    """渲染 paper_card_talk_011 模板。Returns {output_path, video_size_bytes, workdir, http_port}."""
...
def _cli(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="nof render_011",
        description="渲染 paper_card_talk_011 模板出 MP4（自带临时 http.server）",
    )
