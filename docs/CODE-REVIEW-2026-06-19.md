...
| A2 | P1 | 同一份 015 逻辑在 legacy `_execute_*` 与 engine `run_*_step` 各一份，已漂移（lines/storyboard 加了 JSON 重试+domain_image_style，legacy 没有） | `pipeline_runner.py:1457-2440` ↔ `pipeline_performers_final.py` | task-3.1 |
...
| A5 | P1 | rw 默认走 engine → 丢逐模型实时增量进度面板（产物不变，仅 UX 退化，设计已知接受） | `pipeline_performers_final.py:316,397` | task-3.3（决策） |
...
| S5 | P2 | `edit-server.py` kebab-case 违反 Python snake_case | `templates/…/.final-preview-assets/edit-server.py` | task-3.8（低优） |
