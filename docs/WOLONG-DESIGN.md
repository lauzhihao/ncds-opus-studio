...
> ℹ️ **在统一架构（[PRODUCTION-ENGINE-DESIGN.md](PRODUCTION-ENGINE-DESIGN.md)）里**：卧龙 = 生产引擎的
> **app 编排 driver**（沿 沈括→鬼谷子→柳永→吴道子→伯牙→render 派发），本文的调度/闸门/离线学习
> = app 侧的"自治神经层"（rounds_gate / 验收 / retro）规格，仍然有效。本文的"P1–P5"是卧龙自身的实装分期，
> 与生产引擎的 E0–E5 是两套编号，别混。
>
