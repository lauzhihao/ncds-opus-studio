...
> 解说稿提取分支：ASR 片段是唯一脚本候选，OCR 只是视觉证据，不能独立新增文案。黄色/双语字幕优先判为原片对白；
> 白色单行字幕可验证旁白，并在高置信近似但文本冲突时提供复核建议，不能静默覆盖 ASR。原始观测、轨道审计与 commentary artifact 固定落
> `state/works/{platform}/{id}/film_subtitles/v3.*`。
