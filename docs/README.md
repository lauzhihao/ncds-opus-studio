...
**E0 引擎骨架 + E1-a driver API + E1-b1 /instances 路由 + E1-b2 step-performer（lines/storyboard/tts/image/render 真实包装 + 引擎驱动真实 015 链端到端出 mp4 验证）已落地**
（`src/ncds_opus_factory/server/engine/` 全套 driver 原语 + `routes/instances.py` + `pipeline_performers_015.py` + 多轮评审加固，371 passed）；
**下一步 E1-b2 续**=asr/rw 真实包装 + 全局 recipe 绑定 + 路由/前端切换（见设计 §10，风险高那段）。
