...
- 产物(本地 `state/`,NAS 同步是后续)：
  - `state/benchmark/author_<sec_uid>/`：`all_posts.json` + `<aweme_id>.mp4` / `.paraformer.json` / `.txt` + `<aweme_id>/frames/*.jpg` + `collected.json`(采了啥/状态)
...
PYTHONPATH=src python3 -m ncds_opus_factory shenkuo --author <sec_uid> --top 10 [--frames 8]
