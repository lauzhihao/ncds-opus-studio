...
# 完全忽略的文件名 / 通配符
IGNORE_PATTERNS = [
    ".git", ".idea", ".vscode", ".DS_Store", ".worktrees", ".claude",
    "node_modules", "__pycache__", ".pytest_cache", ".venv",
    ".egg-info", "dist", "build",
    ".dart_tool", "Pods", ".symlinks", "ephemeral", ".gradle",  # Flutter/iOS/Android 编译噪音
...
AGENTS = [
    ("wolong",  "卧龙:CEO/操盘手,分段编排派单", "src/ncds_opus_factory/commands/wolong.py"),
    ("shenkuo", "沈括:对标号情报/素材采集", "src/ncds_opus_factory/commands/shenkuo.py"),
    ("guiguzi", "鬼谷子:选题官", "src/ncds_opus_factory/commands/guiguzi.py"),
    ("liuyong", "柳永:抖音爆款编剧(改写+质检)", "src/ncds_opus_factory/commands/liuyong.py"),
    ("wudaozi", "吴道子:配图/分镜", "src/ncds_opus_factory/commands/wudaozi.py"),
    ("boya",    "伯牙:配音/配乐/音效", "src/ncds_opus_factory/commands/boya.py"),
]


def extract_commands_summary() -> list[str]:
    lines = ["**Primitives(原子能力)**", "", "| 命令 | 用途 | 入口 | 状态 |", "|---|---|---|---|"]
    for cmd, desc, rel, note in PRIMITIVES:
        p = PROJECT_ROOT / rel
        status = "ok" if p.exists() else "MISSING"
        lines.append(f"| `{cmd}` | {desc} | `{rel}` | {status}({note}) |")
    lines.extend(["", "**Agents(决策/编排)**", "", "| agent | 职责 | 入口 | 状态 |", "|---|---|---|---|"])
    for name, desc, rel in AGENTS:
        p = PROJECT_ROOT / rel
        status = "ok" if p.exists() else "MISSING"
        lines.append(f"| `{name}` | {desc} | `{rel}` | {status} |")
    return lines
...
def extract_runtime_summary() -> list[str]:
    ...
    gpt_dir = PROJECT_ROOT / "packages" / "core" / "src" / "ncds_opus_core" / "gpt_image"
    if gpt_dir.is_dir():
        py_files = sorted(p.name for p in gpt_dir.glob("*.py"))
        if py_files:
            lines.append("- gpt-image gateway (core): " + ", ".join(py_files))
...
def extract_app_summary() -> list[str]:
    """Flutter app(决策视角,iOS/Android)。"""
    ...
    if feat.is_dir():
        names = sorted(p.name for p in feat.iterdir() if p.is_dir())
        if names:
...
def main() -> int:
    ...
    app_lines = extract_app_summary()
    if app_lines:
        sections.extend(["", "## App (Flutter, 决策视角)", ""])
        sections.extend(app_lines)
