...
1. 抽 3 类 agent 后台任务（enrich/refresh/guiguzi）成独立模块——它们与 final_preview step 执行正交，最先抽、最安全。
...
- 2026-06-22：第一刀先做低风险局部抽取：`rewrite_rw_model` / `refine_rw_model` 共用 `_assert_known_model()` 与 `_rw_mock_short_circuit()`，保留原有错误语义和 mock_final 短路行为。
...
- 2026-06-22：第十一刀继续文件尺寸实降，新增 `server/pipeline_tts_tasks.py` / `PipelineTtsRun` 与 `server/pipeline_render_tasks.py` / `PipelineRenderRun`，把 legacy fallback 的 TTS 与 render 执行上下文一起移出；runner 通过依赖注入传入 `_run_tts_gen_015`、`_rebuild_tts_items_015` 与 `render_final_preview.run`，保留 engine performer / cancel tests 依赖的共享 helper。
...
- 行数变化：`pipeline_runner.py` 维持 195 行；`engine/pipeline_performers_final.py` 从 780 行降到 605 行；本次代码净删 144 行。
