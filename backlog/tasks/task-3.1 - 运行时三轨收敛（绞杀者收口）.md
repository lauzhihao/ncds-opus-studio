...
- **web** → `/jobs/*`（`routes/pipelines.py`）→ PipelineRunner facade →命中节点转 engine `run_step`；
  asr/rw 例外永走 legacy（asr 保留步内增量与 enrich；rw 保留逐模型实时 `model_progress/drafts`）。
...
- 2026-06-22：按 DEC-2 默认策略确认 legacy 仅作冷回退，不补新主链逻辑；`pipeline_runner.py` 标注非 asr/rw legacy `_execute_*` 仅在 `NOF_ENGINE_NODES=legacy/off/none` 或显式排除节点时使用，并记录 A2 已知漂移（legacy lines/storyboard 缺 JSON retry，storyboard 缺 domain_image_style 注入）。
