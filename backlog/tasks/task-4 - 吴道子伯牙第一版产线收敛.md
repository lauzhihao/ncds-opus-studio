...
- 当前 web/engine 主链是 `paper_card_talk_015`，实际节点顺序仍是 `storyboard -> tts -> image -> preview -> render`。
...
- 第一版优先修正产品语义和交互路径，不在本期重写底层 DAG；若需要重排 `image` 到 `tts` 前，单列后续决策任务。
...
- 不把 `image` 节点在 backend DAG 中移动到 `tts` 前；第一版通过前端阶段映射和自动推进解决用户体验。
...
- [ ] #4 伯牙面板包装当前 TTS，能展示声音产物、重试状态，并把完成态交给后续画面补齐/成片流程
