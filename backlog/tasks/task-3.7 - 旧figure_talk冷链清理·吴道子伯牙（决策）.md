...
- `commands/wudaozi.py`（450 行）/ `commands/boya.py`（482 行）+ 各 `_test.py` —— 完整旧实现（非 stub），
  产 figure_talk/剪影成片。**但 live 生产线（web 画布 + engine）走 storyboard→tts→image→render_final_preview，
  完全不经 wudaozi/boya**。
- 附属冷链：`templates/figure_talk/`、`templates/stickman/`、`scripts/scan_figure_lib.py`、
  ...
  core `render.py`（非 final_preview 录屏器）、factory `templates/paper_card_talk`（非 final_preview 目录）。
...
- 2026-06-22：在 `commands/wudaozi.py` 与 `commands/boya.py` 顶部标注旧 figure_talk 冷链边界；2026-06-23 同步当前 web/engine 主链为 `final_preview` 的 `storyboard -> image -> tts -> render_final_preview`，后续重写吴道子/伯牙/render 下游时直接替换，不为旧实现加兼容层。
